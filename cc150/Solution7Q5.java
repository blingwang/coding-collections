...
class Square {
     ...
     * intercepts the edge of square 1. That is, draw a line from mid2 
    ...
    public Point extend(Point mid1, Point mid2, double size){
         ...
         * calculation will throw a divide by 0 exeption. So we compute
         * this specially
         */
        if (mid1.x == mid2.x) {
         ...
         * y axis. If the slope is "shallow" (<1) the end of the line 
         * segment will hit size/2 units away from the middle on the 
         * x axisl
         */
        if (Math.abs(slope) == 1) {
    ...
    public Line cut(Square other) {
        // Calculate where a line between each middle would collide with
        ...
        // Of abobe points, find start and end of lines. Start is 
        // farthest left (with top most as a tie breaker) and end is 
        // farthest right ( with bottom most as a tie breaker.
        Point start = p1;
        Point end = p1;
